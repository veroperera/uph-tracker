...
const Tag = ({color,children}) => (
  <span style={{display:"inline-block",padding:"2px 10px",fontSize:"10px",letterSpacing:"2px",textTransform:"uppercase",background:color+"22",color,border:`1px solid ${color}55`,fontFamily:"'IBM Plex Mono',monospace"}}>{children}</span>
);
const Box = ({label,value,color="#666",small}) => (
  <div style={{background:"#17191F",border:"1px solid #1E2028",padding:"16px 20px",position:"relative",overflow:"hidden"}}>
...
export default function UPHApp() {
  ...
  const C=sesionActiva?.unidadColor||"#E87C2E";
  const efC=(e)=>e>=100?"#4CAF50":e>=80?"#E8C040":"#E84040";
  ...
  const G=`
    ...
    *{box-sizing:border-box;margin:0;padding:0}body{background:#0F1014}
    ::-webkit-scrollbar{width:4px}::-webkit-scrollbar-thumb{background:#2a2a2a}
    input,select,button{outline:none;font-family:'IBM Plex Mono',monospace}select option{background:#17191F}
    ...
    .inp:focus{border-color:var(--ac)}.inp::placeholder{color:#333}
    ...
    .btn-ac:hover{filter:brightness(1.15);transform:translateY(-1px)}.btn-ac:disabled{opacity:.5;cursor:not-allowed;transform:none}
    .btn-ghost{background:transparent;border:1px solid #2a2a2a;color:#777}.btn-ghost:hover{border-color:#555;color:#ccc}
    ...
    .nav-item:hover{color:#999}.nav-item.on{color:#E8E6E0;border-bottom-color:var(--ac)}
    ...
    @keyframes blink{0%,100%{opacity:1}50%{opacity:.5}}.blink{animation:blink 1.4s infinite}
    ...
    .area-card:hover{border-color:#333}.area-card.sel{border-color:var(--card-ac)}
    ...
    .mpholder{background:#17191F;border:1px dashed #252830;padding:28px;display:flex;align-items:center;justify-content:center;flex-direction:column;gap:8px}
    .toast{position:fixed;bottom:24px;left:50%;transform:translateX(-50%);padding:12px 24px;font-family:'IBM Plex Mono',monospace;font-size:11px;letter-spacing:2px;z-index:9999;white-space:nowrap}
  `;

  return (
    <div style={{minHeight:"100vh",background:"#0F1014",fontFamily:"'IBM Plex Mono',monospace",color:"#E8E6E0","--ac":C}}>
      <style>{G}</style>

      {toast&&<div className="toast" style={{background:toast.ok?"#1a2e1a":"#2e1a1a",border:`1px solid ${toast.ok?"#4CAF50":"#e74c3c"}`,color:toast.ok?"#4CAF50":"#e74c3c"}}>{toast.msg}</div>}
      ...
      <div style={{background:"#0A0B0E",borderBottom:"1px solid #1E2028",padding:"14px 20px",display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:100}}>
        <div style={{display:"flex",alignItems:"center",gap:"10px"}}>
          <div style={{width:"28px",height:"28px",background:C,clipPath:"polygon(0 0,100% 0,100% 65%,65% 100%,0 100%)"}}/>
          <div>
            <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:"20px",letterSpacing:"3px"}}>UPH TRACKER</div>
            <div style={{fontSize:"9px",color:"#3a3a3a",letterSpacing:"2px"}}>CONTROL DE PRODUCTIVIDAD · NVS CANCUN</div>
          </div>
        </div>
        <nav style={{display:"flex",gap:"2px"}}>
          {!adminAuth&&["inicio","historial"].map(v=>(
            <button key={v} className={`nav-item ${vista===v||(v==="inicio"&&vista==="sesion")?"on":""}`}
              style={{"--ac":C}} onClick={()=>{if(!sesionActiva)setVista(v);}}>
              {v==="inicio"?"Nueva":"Historial"}
            </button>
          ))}
          {adminAuth
            ?<button className="btn btn-danger" style={{fontSize:"10px"}} onClick={salirAdmin}>✕ Salir Admin</button>
            :<button className="nav-item" style={{"--ac":"#888",color:vista==="admin"?"#E8E6E0":"#333"}} onClick={()=>setVista("admin")}>⚙ Admin</button>
          }
        </nav>
      </div>

      <div style={{maxWidth:"740px",margin:"0 auto",padding:"28px 18px","--ac":C}}>
          ...
          <div>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:"18px"}}>
                ...
                <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:"10px"}}>
                  {areas.map(a=>(
                    <div key={a.id} className={`area-card ${form.areaId===a.id?"sel":""}`}
                      style={{"--card-ac":a.color}} onClick={()=>setForm(f=>({...f,areaId:a.id}))}>
              ...
              <div style={{maxWidth:"260px"}}>
                ...
                <input type="number" className="inp" placeholder="Ej. 500" min="1" value={form.unidades} onChange={e=>setForm(f=>({...f,unidades:e.target.value}))}/>
              </div>
        ...
        {vista==="sesion"&&sesionActiva&&(
          <div style={{"--ac":C}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"24px"}}>
              ...
              <Tag color={C}>{sesionActiva.unidadLabel}</Tag>
            </div>

            <div style={{textAlign:"center",background:"#17191F",border:"1px solid #1E2028",padding:"32px 20px",marginBottom:"20px"}}>
              {pausada&&<div className="blink" style={{fontSize:"9px",letterSpacing:"3px",color:"#E8A040",marginBottom:"10px"}}>⏸ EN PAUSA</div>}
              <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:"80px",letterSpacing:"4px",color:C,lineHeight:1,textShadow:`0 0 40px ${C}44`}}>
                {formatTime(elapsed)}
              </div>
              {pauseTotal>0&&<div style={{fontSize:"10px",color:"#444",marginTop:"8px"}}>Pausa acumulada: {formatTime(pauseTotal)}</div>}
            </div>

            {snapshot?(
              <div>
                <div style={{fontSize:"9px",color:"#555",letterSpacing:"2px",marginBottom:"10px",textAlign:"center"}}>— MÉTRICAS AL MOMENTO DE PAUSA —</div>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"10px",marginBottom:"16px"}}>
                  <Box label="UPH REAL" value={snapshot.uphReal} color={C}/>
                  <Box label="UPH ESTÁNDAR" value={sesionActiva.uphEstandar} color="#444"/>
                  <Box label="EFICIENCIA" value={`${snapshot.eficiencia}%`} color={efC(snapshot.eficiencia)}/>
                </div>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"10px",marginBottom:"16px"}}>
                  <Box label="UNIDADES OBJETIVO" value={sesionActiva.unidades.toLocaleString()} color="#333" small/>
                  <Box label="TIEMPO ACTIVO" value={formatTime(snapshot.elapsed)} color="#333" small/>
                </div>
                <div style={{height:"4px",background:"#1E2028",marginBottom:"22px"}}>
                  <div style={{height:"100%",width:`${Math.min(snapshot.eficiencia,100)}%`,background:efC(snapshot.eficiencia),transition:"width .6s"}}/>
                </div>
              </div>
            ):(
              <div className="mpholder" style={{marginBottom:"22px"}}>
            ...
            <div style={{display:"flex",gap:"10px"}}>
              <button onClick={togglePausa} style={{flex:1,padding:"14px",fontWeight:700,fontSize:"11px",letterSpacing:"2px",textTransform:"uppercase",cursor:"pointer",transition:"all .15s",background:pausada?"#2A1F10":"#1E2028",border:`1px solid ${pausada?"#E87C2E":"#333"}`,color:pausada?"#E87C2E":"#E8E6E0"}}>
                {pausada?"▶ Reanudar":"⏸ Pausa"}
              </button>
              <button className="btn btn-ac" style={{"--ac":C,flex:2}} onClick={terminar} disabled={guardando}>
        ...
        {vista==="historial"&&(
          <div>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",marginBottom:"24px",flexWrap:"wrap",gap:"12px"}}>
              <div>
                <div style={{fontSize:"9px",color:"#3a3a3a",letterSpacing:"3px",marginBottom:"6px"}}>//  Sesiones del dispositivo</div>
                <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:"34px",letterSpacing:"2px"}}>Historial</div>
              </div>
              <div style={{display:"flex",flexDirection:"column",gap:"6px",alignItems:"flex-end"}}>
                <div style={{fontSize:"9px",color:"#555",letterSpacing:"1px"}}>📊 Historial completo → Google Sheets</div>
                ...
                <div key={s.id||i} className="row-hist">
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:"8px"}}>
                    ...
                    <div style={{textAlign:"right"}}>
                      <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:"36px",color:efC(s.eficiencia),lineHeight:1}}>{s.eficiencia}%</div>
                  ...
                  <div style={{height:"3px",background:"#1E2028",marginTop:"10px"}}>
                    <div style={{height:"100%",width:`${Math.min(s.eficiencia,100)}%`,background:efC(s.eficiencia),transition:"width .6s"}}/>
                  </div>
                </div>
              ))
            }
        ...
        {vista==="admin"&&!adminAuth&&(
          <div style={{maxWidth:"360px",margin:"60px auto"}}>
            ...
            {pinError&&<div style={{fontSize:"10px",color:"#e74c3c",marginTop:"8px"}}>PIN incorrecto.</div>}
          </div>
        )}
          ...
          <div style={{"--ac":"#E87C2E"}}>
            ...
            <div style={{display:"flex",gap:"2px",marginBottom:"28px",borderBottom:"1px solid #1E2028"}}>
              {[["dashboard","📊 Dashboard"],["empleados","👷 Empleados"],["areas","📦 Áreas / UPH"],["historial","📋 Historial"]].map(([v,l])=>(
                <button key={v} className={`nav-item ${adminVista===v?"on":""}`} style={{"--ac":"#E87C2E"}} onClick={()=>setAdminVista(v)}>{l}</button>
              ))}
            </div>
            ...
            {adminVista==="dashboard"&&(
              <div>
                <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:"12px",marginBottom:"24px"}}>
                  <Box label="SESIONES HOY" value={sesiones.filter(s=>fmtFecha(s.inicio)===fmtFecha(Date.now())).length} color="#E87C2E"/>
                  <Box label="EMPLEADOS ACTIVOS" value={empleados.filter(e=>e.activo).length} color="#4A90D9"/>
                  <Box label="EFIC. PROMEDIO" value={sesiones.length?`${Math.round(sesiones.reduce((a,s)=>a+s.eficiencia,0)/sesiones.length)}%`:"—"} color="#4CAF50"/>
                </div>
                <div style={{background:"#17191F",border:"1px solid #1E2028",padding:"20px",marginBottom:"16px",borderLeft:"3px solid #4A90D9"}}>
                  <div style={{fontSize:"10px",color:"#4A90D9",letterSpacing:"2px",marginBottom:"8px"}}>📊 HISTORIAL COMPLETO EN GOOGLE SHEETS</div>
                  <div style={{fontSize:"11px",color:"#666",lineHeight:1.7}}>Cada sesión terminada se guarda automáticamente en tu hoja de cálculo. Ábrela desde Google Drive para ver el historial completo de todos los empleados y dispositivos.</div>
                </div>
                {areas.map(a=>{
                  ...
                  return(
                    <div key={a.id} className="card" style={{marginBottom:"10px",borderLeft:`3px solid ${a.color}`}}>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                        ...
                        <div style={{textAlign:"right"}}>
                          <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:"28px",color:efC(avgEf)}}>{avgEf}%</div>
            ...
            {adminVista==="empleados"&&(
              <div>
                ...
                {empleados.length===0
                  ?<div style={{textAlign:"center",color:"#2a2a2a",padding:"40px 0",fontSize:"11px",letterSpacing:"2px"}}>Sin empleados. Agrega el primero.</div>
                  :empleados.map(e=>{
                    ...
                    return(
                      <div key={e.id} className="emp-row">
                        ...
                        <div style={{display:"flex",gap:"8px"}}>
                          <button className="btn btn-ghost" style={{fontSize:"9px",padding:"7px 12px"}} onClick={()=>setModalEmp({...e})}>Editar</button>
                          <button className="btn btn-danger" onClick={()=>setEmpleados(p=>p.map(x=>x.id===e.id?{...x,activo:!x.activo}:x))}>
              ...
              <div>
                ...
                {areas.map(a=>(
                  <div key={a.id} className="emp-row" style={{borderLeft:`3px solid ${a.color}`}}>
                    ...
                    <div style={{display:"flex",gap:"8px"}}>
                      <button className="btn btn-ghost" style={{fontSize:"9px",padding:"7px 12px"}} onClick={()=>setModalArea({...a})}>Editar</button>
                      <button className="btn btn-danger" onClick={()=>{if(areas.length>1)setAreas(p=>p.filter(x=>x.id!==a.id));}}>Eliminar</button>
                    </div>
                  </div>
                ))}
              </div>
            ...
            {adminVista==="historial"&&(
              <div>
                <div style={{background:"#17191F",border:"1px solid #4A90D944",padding:"16px 20px",marginBottom:"20px",borderLeft:"3px solid #4A90D9"}}>
                  <div style={{fontSize:"10px",color:"#4A90D9",letterSpacing:"2px",marginBottom:"6px"}}>📊 HISTORIAL CENTRALIZADO EN GOOGLE SHEETS</div>
                  <div style={{fontSize:"11px",color:"#666",lineHeight:1.6}}>El historial completo de <strong style={{color:"#E8E6E0"}}>todos los dispositivos</strong> está en tu Google Sheets. Lo que se muestra aquí es solo el registro de este dispositivo.</div>
                </div>
                {sesiones.length===0
                  ?<div style={{textAlign:"center",color:"#2a2a2a",padding:"60px 0",fontSize:"11px",letterSpacing:"2px"}}>Sin sesiones en este dispositivo</div>
                  :sesiones.map((s,i)=>(
                    <div key={s.id||i} className="row-hist">
                      <div style={{display:"flex",justifyContent:"space-between",flexWrap:"wrap",gap:"8px"}}>
                        ...
                        <div style={{textAlign:"right"}}>
                          <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:"32px",color:efC(s.eficiencia),lineHeight:1}}>{s.eficiencia}%</div>
                        </div>
                      </div>
        ...
        <div className="modal-bg" onClick={()=>setModalEmp(null)}>
          <div className="modal" onClick={e=>e.stopPropagation()}>
            <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:"22px",letterSpacing:"2px",marginBottom:"20px"}}>
              {modalEmp.id?"Editar Empleado":"Nuevo Empleado"}
            </div>
            <div style={{display:"flex",flexDirection:"column",gap:"14px"}}>
              ...
              <div style={{display:"flex",gap:"10px",marginTop:"8px"}}>
                <button className="btn btn-ghost" style={{flex:1}} onClick={()=>setModalEmp(null)}>Cancelar</button>
